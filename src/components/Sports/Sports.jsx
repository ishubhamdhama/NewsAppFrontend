...
function Sports() {
    ...
    useEffect(()=>{
        const fetchData = async()=> {
            try {
                const response = await fetch("https://newsapi.org/v2/top-headlines?country=us&category=sports&apiKey=57383e0a076a459486f0397fce7a3495")
                if(!response.ok) {
                    throw new Error('This Response is not ok')
                }
                const result = await response.json();
                setSportsData(result.articles)
                setLoading(false)
            } catch (error) {
  ...
  return (
    ...
    {loading && <div className="text-center h-[100vh] flex justify-center items-center" style={{fontSize:"4rem"}}><i className="fa-solid fa-arrows-rotate fa-spin fa-2xl"></i></div>}
            <h1 className="text-center text-gray-900 text-5xl font-bold py-6">Sports</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 back">
                    {sportsData.slice(0, 18).map((data, index) => (
  ...
  {/* Card content */}
                                        ...
                                        <CardItem translateZ="100" className="w-full mt-4">
                                            {data.urlToImage ? (
                                                ...
                                                alt="" 
                                                onError={(e)=>{
                                                    e.currentTarget.src= 'https://images.pexels.com/photos/46798/the-ball-stadion-football-the-pitch-46798.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'
                                                }}/>
                                            ) : (
                                                <img src="https://images.pexels.com/photos/9153468/pexels-photo-9153468.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt="Placeholder" style={{ height: "300px", width: "400px" }} /> // Render placeholder image if urlToImage is null
                                            )}
                                        </CardItem>
