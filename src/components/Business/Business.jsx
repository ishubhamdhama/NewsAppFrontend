...
function Business() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch("https://newsapi.org/v2/everything?q=business&sortBy=publishedAt&apiKey=57383e0a076a459486f0397fce7a3495");
                if (!response.ok) {
                    throw new Error("Response is not ok")
                }
                const result = await response.json();
                setGetBusinessData(result.articles)
                setLoading(false);
    ...
    return (
        <>
            ...
            <h1 className="text-center text-gray-900 text-5xl font-bold py-6">Business</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 back">
                    {getBusinessData.slice(0, 99).map((data, index) => (
  ...
  {/* Card content */}
                                        ...
                                        <CardItem translateZ="100" className="w-full mt-4">
                                            {data.urlToImage ? (
                                                ...
                                                alt="" 
                                                onError={(e)=>{
                                                    e.currentTarget.src = "https://images.pexels.com/photos/2127039/pexels-photo-2127039.jpeg?auto=compress&cs=tinysrgb&w=600"
                                                }}/>
                                            ) : (
                                                <img src="https://images.pexels.com/photos/5561912/pexels-photo-5561912.jpeg?auto=compress&cs=tinysrgb&w=600" alt="Placeholder" style={{ height: "300px", width: "400px" }} /> // Render placeholder image if urlToImage is null
                                            )}
                                        </CardItem>
